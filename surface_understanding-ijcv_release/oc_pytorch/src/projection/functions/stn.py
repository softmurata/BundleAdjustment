...
class VTNFunction(Function):
    def forward(self, input1, input2):
        ...
        if not input1.is_cuda:
            my_lib.BilinearSamplerBZHWD_updateOutput(input1, input2, output)
        else:
            output = output.cuda()
            my_lib.BilinearSamplerBZHWD_updateOutput_cuda(input1, input2, output)
        return output
    ...
    def backward(self, grad_output):
        ...
        print('cuda: ? : ' + str(grad_output.is_cuda))
        if not grad_output.is_cuda:
            my_lib.BilinearSamplerBZHWD_updateGradInput(self.input1, self.input2, grad_input1, grad_input2, grad_output)
        else:
            grad_input1 = grad_input1.cuda()
            grad_input2 = grad_input2.cuda()
            my_lib.BilinearSamplerBZHWD_updateGradInput_cuda(self.input1, self.input2, grad_input1, grad_input2, grad_output)
        return grad_input1, grad_input2
