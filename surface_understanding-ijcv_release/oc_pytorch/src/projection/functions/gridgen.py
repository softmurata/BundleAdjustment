...
class PersGridGenFunction(Function):
    def __init__(self, height, width, depth):
        super(PersGridGenFunction, self).__init__()
        self.height, self.width, self.depth = height, width, depth
        self.grid = np.zeros( [self.height, self.width, self.depth, 4], dtype=np.float32)
        self.grid[:,:,:,1] = np.expand_dims(np.repeat(
            np.expand_dims(np.repeat(np.expand_dims(np.linspace(-1, 1, self.height), 0), repeats = self.width, axis = 0).T, 0),
            repeats=self.depth, axis=0), 0)
        self.grid[:,:,:,2] = np.expand_dims(np.repeat(
            np.expand_dims(np.repeat(np.expand_dims(np.linspace(-1, 1, self.width), 0), repeats = self.height, axis = 0), 0),
            repeats=self.depth, axis=0), 0)
        self.grid[:,:,:,0] = np.repeat(np.expand_dims(np.linspace(-1, 1, height), 0), repeats = width*height, axis = 1).reshape((width,height,depth))
        self.grid[:,:,:,3] = np.ones([self.height, width, depth])
    ...
    def forward(self, input1):
        ...
        for i in range(input1.size(0)):
                output = torch.bmm(self.batchgrid.view(-1, self.height*self.width*self.depth, 4), torch.transpose(input1, 1, 2)).view(-1, self.height, self.width, self.depth, 3)
