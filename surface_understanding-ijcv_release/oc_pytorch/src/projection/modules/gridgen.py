...
import numpy as np
from projection.functions.gridgen import AffineGridGenFunction, CylinderGridGenFunction, PersGridGenFunction
...
class PersGridGen(Module):
    def __init__(self, height, width, depth):
        super(PersGridGen, self).__init__()
        self.height, self.width, self.depth = height, width, depth

        self.f = PersGridGenFunction(self.height, self.width, self.depth)
...
class AffineGridGen(Module):
    ...
    def forward(self, input):
        if not self.aux_loss:
            ...
            print(input.size(), batch_identity.size())
...
class DenseAffineGridGen(Module):
    ...
    def forward(self, input1):
        ...
        output = torch.cat([x.sum(3, keepdim=True),y.sum(3, keepdim=True)], 3)
        return output
...
class DenseAffine3DGridGen(Module):
    ...
    def forward(self, input1):
        ...
        x = torch.sum(torch.mul(self.batchgrid3d, input1[:,:,:,0:4]), 3, keepdim=True)
        y = torch.sum(torch.mul(self.batchgrid3d, input1[:,:,:,4:8]), 3, keepdim=True)
        z = torch.sum(torch.mul(self.batchgrid3d, input1[:,:,:,8:]), 3, keepdim=True)
        #print(x)
